...
def save_and_plot_results(quant_maps, output_folder):
    ...
    mask = np.load('../dot_prod_example/mask_3T.npy')

    fig_fn = os.path.join(output_folder, 'deep_reco_clinical.eps')
    ...
    # ksw (Hz)
    plt.subplot(122)
    plt.imshow(quant_maps['ksw']*mask, cmap='magma', clim=(0, 1400))
    plt.colorbar(ticks=np.arange(0, 1401, 100), fraction=0.046, pad=0.04)
