...
class ConfigParams:
    ...
    # Water_pool
    water_t1 = np.arange(1200, 2300 + 100, 100) / 1000  # np.arange(1300, 2300 + 100, 100) / 1000
    water_t1 = water_t1.tolist()  # vary t1

    water_t2 = np.arange(40, 160 + 20, 20) / 1000  # np.arange(40, 110 + 10, 10) / 1000
    ...
    cest_mt_sol_conc = np.arange(2e3, 30e3 + 2e3, 2e3)
    cest_mt_protons = 1
    ...
    cest_amine_t2 = [1 / 1000]  # fix solute t2
    cest_amine_k = np.arange(30, 70+10, 10).tolist()  # np.arange(40, 46+3, 3).tolist()  # fixed mean  #
    cest_amine_dw = 3.5  # fixed solute exchange rate at 3.5 ppm

    # solute concentration * protons / water concentration
    cest_amine_sol_conc = np.arange(300, 500 + 50, 50)  # np.arange(320, 340 + 20, 20)
    ...
    def __init__(self, fp_prtcl_name, general_fn):
        ...
        dict_folder = os.path.join(general_fn, 'exp', 'mt_amide_glu_dicts', dict_name_category, 'amide', fp_prtcl_name)
...
def main():
    ...
    # Protocol stats:
    fp_prtcl_names = ['51_Amide']  # should i take the other version of it? '51_Amide_rs'
    ...
    for phantom_i in range(len(glu_phantom_fns)):
        ...
        for fp_prtcl_name in fp_prtcl_names:
            ...
            generate_mrf_cest_dictionary(seq_fn=seq_fn, param_fn=yaml_fn, dict_fn=dict_fn, num_workers=cfg.num_workers,
                                         mt_flag=3, axes='xy')  # axes can also be 'z' if no readout is simulated
