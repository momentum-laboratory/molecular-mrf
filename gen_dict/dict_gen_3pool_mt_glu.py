...
class ConfigParams:
    ...
    # Water_pool
    water_t1 = np.arange(1200, 2325 + 75, 75) / 1000  # np.arange(1300, 2300 + 100, 100) / 1000
    water_t1 = water_t1.tolist()  # vary t1

    water_t2 = np.arange(40, 160 + 15, 15) / 1000  # np.arange(40, 110 + 10, 10) / 1000
    ...
    cest_amine_t1 = [1200 / 1000]  # fix solute t1
    cest_amine_t2 = [7 / 1000]  # fix solute t2
    # cest_amine_k = np.arange(5000, 12500+125, 125).tolist()  # vary solute exchange rate (I wanna get to 10000) np.arange(8000, 12500+250, 250).tolist()  #
    # cest_amine_k = [4000, 4250, 4500, 4750, 5000, 5250, 5500, 5750, 6000,
    ...
    #                 10250, 10500, 10750, 11000,
    #                 11250, 11500, 11750, 12000,
    #                 12250, 12500, 12750]
    # cest_amine_k = np.arange(8500, 8500+100, 100).tolist()
    cest_amine_k = [0, 8500]
    cest_amine_dw = 3  # fixed solute exchange rate at 3 ppm

    # solute concentration * protons / water concentration
    cest_amine_sol_conc = np.arange(0, 500 + 50, 50)  # solute concentration + 11
    ...
    # print(f'Your poison is {len(water_t1)*len(water_t2)*len(cest_mt_k)*len(cest_mt_f)*len(cest_amine_k)*len(cest_amine_f)} long')
    ...
    def __init__(self, fp_prtcl_name, general_fn):
        ...
        dict_name_category = 'new_t'
        dict_title = 'fixed_8500_high_conc_skip'
        idx = 1
        dict_folder = os.path.join(general_fn, 'exp', 'mt_glu_dicts', dict_name_category, 'glu', dict_title, fp_prtcl_name)
        if not os.path.exists(dict_folder):
            os.makedirs(dict_folder)
        # self.yaml_fn = os.path.join(dict_folder, f'scenario_{idx}.yaml')
        # self.seq_fn = os.path.join(dict_folder, f'acq_protocol_{idx}.seq')
        # self.dict_fn = os.path.join(dict_folder, f'dict_{idx}.pkl')  # pickle format!
...
def main():
    ...
    # Protocol stats:
    fp_prtcl_names = ['107a'] # 107a, 51_glu
    ...
    for phantom_i in range(len(glu_phantom_fns)):
        ...
        for fp_prtcl_name in fp_prtcl_names:
            ...
            generate_mrf_cest_dictionary(seq_fn=seq_fn, param_fn=yaml_fn, dict_fn=dict_fn, num_workers=cfg.num_workers,
                                         mt_flag=2, axes='xy')  # axes can also be 'z' if no readout is simulated
