...
from cest_mrf.write_scenario import write_yaml_4pool
from cest_mrf.dictionary.generation import generate_mrf_cest_dictionary_4pool
...
class ConfigParams:
    ...
    gamma = 267.5153  # gamma for H [rad/uT] = gyro_ratio_hz*2pi
    b0_inhom = 0
    rel_b1 = 1
    ...
    cest_mt_t1 = [0 / 1000]  # fix solute t1 (copies water t1s)
    cest_mt_t2 = [0.04 / 1000]  # fix solute t2
    cest_mt_k = np.arange(5, 100 + 15, 15)
    cest_mt_k = cest_mt_k.tolist()

    cest_mt_dw = -2.5  # fixed solute exchange rate at -2.5 ppm

    # solute concentration * protons / water concentration
    cest_mt_sol_conc = np.arange(2e3, 30e3 + 4e3, 4e3)
    cest_mt_protons = 1
    cest_mt_water_conc = 110000
    cest_mt_f = cest_mt_sol_conc * cest_mt_protons / cest_mt_water_conc
    cest_mt_f = cest_mt_f.tolist()

    # Second CEST pool (Amine solute pool)
    cest_amide_t1 = [0 / 1000]  # fix solute t1 (copies water t1s)
    cest_amide_t2 = [1 / 1000]  # fix solute t2
    cest_amide_k = np.arange(10, 100+10, 10).tolist()  # np.arange(43, 43+5, 5).tolist()  # fixed mean  #
    cest_amide_dw = 3.5  # fixed solute exchange rate at 3.5 ppm

    # solute concentration * protons / water concentration
    cest_amide_sol_conc = np.arange(100, 1000 + 100, 100)  # np.arange(330, 330 + 1, 1)  # solute concentration (not sure about this one!) #
    cest_amide_protons = 1
    cest_amide_water_conc = 110000
    cest_amide_f = cest_amide_sol_conc * cest_amide_protons / cest_amide_water_conc
    cest_amide_f = cest_amide_f.tolist()

    # Third CEST pool (Glu solute pool)
    cest_glu_t1 = [1200 / 1000]  # fix solute t1
    cest_glu_t2 = [7 / 1000]  # fix solute t2
    # cest_glu_k = np.arange(6500, 12000+100, 100).tolist()
    # cest_glu_k = [0, 6500, 7500, 8500, 9500, 10500, 11500, 12500]
    cest_glu_k = [0, 5500]
    cest_glu_dw = 3  # fixed solute exchange rate at 3 ppm

    # solute concentration * protons / water concentration
    cest_glu_sol_conc = np.arange(0, 50 + 5, 5)  # solute concentration
    cest_glu_protons = 3
    cest_glu_water_conc = 110000
    cest_glu_f = cest_glu_sol_conc * cest_glu_protons / cest_glu_water_conc
    cest_glu_f = cest_glu_f.tolist()
    ...
    def __init__(self, fp_prtcl_name, general_fn):
        ...
        dict_name_category = '4pool'
        dict_folder = os.path.join(general_fn, 'exp', 'mt_amide_glu_dicts', dict_name_category, 'glu', fp_prtcl_name)
...
def main():
    ...
    # Protocol stats:
    fp_prtcl_names = ['107a', '51_Amide']  # , '51_Amide'
    ...
    for phantom_i in range(len(glu_phantom_fns)):
        ...
        for fp_prtcl_name in fp_prtcl_names:
            ...
            generate_mrf_cest_dictionary_4pool(seq_fn=seq_fn, param_fn=yaml_fn, dict_fn=dict_fn,
                                               num_workers=cfg.num_workers, axes='xy', add_iter=6)  # axes can also be 'z' if no readout is simulated
