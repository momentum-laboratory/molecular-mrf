...
from sequential_nn_example.dataset import GluDataset
...
def main():
    ...
    dict_name = 'mouse_mt_amide_rs_diluted'
    net_name = 'mouse_mt_amide_rs_diluted_noise_01'
    fp_prtcl_name = '51_Amide_rs'

    current_dir = os.getcwd()  # Get the current directory
    parent_dir = os.path.dirname(current_dir)  # Navigate up one directory level
    amide_dict_fn = os.path.join(parent_dir, 'data', 'exp', '3_seq', dict_name, fp_prtcl_name, 'dict.pkl')  # dict directory
    nn_fn = os.path.join(current_dir, 'mouse_nns', 'amide_nns',  f'{dict_name}', f'{net_name}.pt')  # nn directory
    ...
    (min_param_tensor, max_param_tensor, min_water_t1t2_tensor, max_water_t1t2_tensor,
     min_mt_param_tensor, max_mt_param_tensor) = define_min_max(amide_dict_fn)

    # Loading the training dataset
    training_data = pd.read_pickle(amide_dict_fn)
...
def define_min_max(glu_dict_fn):
    ...
    min_param_tensor = torch.tensor(np.hstack((min_fs, min_ksw)), requires_grad=False)  # can be switched to  min_fs, min_ksw
    ...
    min_mt_param_tensor = torch.tensor(np.hstack((min_mt_fs, min_mt_ksw)), requires_grad=False)  # can be switched to  min_fs, min_ksw
