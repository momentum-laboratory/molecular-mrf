...
from sequential_nn_example.dataset import Dataset_4pool
from sequential_nn_example.model import Network_4pool
...
def main():
    ...
    dict_name = 'mouse_mt_amine_glu_diluted'
    net_name = 'mouse_mt_amine_glu_diluted_noise_005'
    fp_prtcl_name = '107a'

    current_dir = os.getcwd()  # Get the current directory
    parent_dir = os.path.dirname(current_dir)  # Navigate up one directory level
    glu_dict_fn = os.path.join(parent_dir, 'data', 'exp', '3_seq', dict_name, fp_prtcl_name, 'dict.pkl')  # dict directory
    nn_fn = os.path.join(current_dir, 'mouse_nns', 'glu_nns', f'{dict_name}', f'{net_name}.pt')  # nn directory
    ...
    (min_param_tensor, max_param_tensor, min_water_t1t2_tensor, max_water_t1t2_tensor,
     min_mt_param_tensor, max_mt_param_tensor, min_amine_param_tensor, max_amine_param_tensor) = define_min_max(glu_dict_fn)

    # Loading the training dataset
    training_data = pd.read_pickle(glu_dict_fn)
    train_dataset = Dataset_4pool(training_data)
    del training_data
    ...
    reco_net = train_network(train_loader, device, sched_iter, learning_rate, num_epochs, noise_std, patience,
                             ...
                             max_water_t1t2_tensor, min_mt_param_tensor, max_mt_param_tensor,
                             min_amine_param_tensor, max_amine_param_tensor, nn_fn)
...
def train_network(train_loader, device, sched_iter, learning_rate, num_epochs, noise_std, patience, min_delta,
                  min_param_tensor, max_param_tensor, min_water_t1t2_tensor, max_water_t1t2_tensor,
                  min_mt_param_fs_ksw, max_mt_param_fs_ksw, min_amine_param_fs_ksw, max_amine_param_fs_ksw, nn_fn
                  ):
    ...
    reco_net = Network_4pool(sched_iter, n_hidden=2, n_neurons=300).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for counter, dict_params in enumerate(train_loader, 0):
            cur_fs, cur_ksw, cur_t1w, cur_t2w, cur_mt_fs, cur_mt_ksw, cur_amine_fs, cur_amine_ksw, cur_norm_sig = dict_params
            ...
            input_amine_fs_ksw = torch.stack((cur_amine_fs, cur_amine_ksw), dim=1)
            ...
            input_amine_fs_ksw = normalize_range(original_array=input_amine_fs_ksw, original_min=min_amine_param_fs_ksw,
                                              original_max=max_amine_param_fs_ksw, new_min=0, new_max=1)
            ...
            # adding the amine/mt/t1/t2 input as additional elements in the noised_sig vector
            noised_sig = torch.hstack((input_amine_fs_ksw, input_mt_fs_ksw, input_water_t1t2, noised_sig))
        ...
        if epoch % 25 == 0:
            ...
            torch.save({
                ...
                'input_order': ['amide_fs', 'amide_ksw', 'mt_fs', 'mt_ksw', 't1', 't2'],
                'epoch': epoch
            }, nn_fn)
...
def define_min_max(glu_dict_fn):
    ...
    min_fs = np.min(dictionary['fs_1'])  # uncomment if non-zero minimum limit is required
    min_ksw = np.min(dictionary['ksw_1'].transpose().astype(float))  # uncomment if non-zero minimum limit needed
    max_fs = np.max(dictionary['fs_1'])
    max_ksw = np.max(dictionary['ksw_1'].transpose().astype(float))
    ...
    min_mt_fs = np.min(dictionary['fs_2'])
    min_mt_ksw = np.min(dictionary['ksw_2'].transpose().astype(float))
    max_mt_fs = np.max(dictionary['fs_2'])
    max_mt_ksw = np.max(dictionary['ksw_2'].transpose().astype(float))

    min_amine_fs = np.min(dictionary['fs_0'])
    min_amine_ksw = np.min(dictionary['ksw_0'].transpose().astype(float))
    max_amine_fs = np.max(dictionary['fs_0'])
    max_amine_ksw = np.max(dictionary['ksw_0'].transpose().astype(float))
    ...
    min_amine_param_tensor = torch.tensor(np.hstack((0, 0)), requires_grad=False)  # can be switched to  min_fs, min_ksw
    max_amine_param_tensor = torch.tensor(np.hstack((max_amine_fs, max_amine_ksw)),
                                       requires_grad=False)  # can be switched to  min_fs, min_ksw
