...
from sequential_nn_example.model import Network_aacid
...
def main():
    ...
    dict_name = 'mouse_aacid_k'
    net_name = 'mouse_aacid_k_noise_01'  # 'mouse_20_noise_01'
    fp_prtcl_name = '107a'
    ...
    batch_size = 1024
    num_epochs = 150
    noise_std = 0.01  # noise level for training, 0.01, 0.005, 0.001
...
def train_network(train_loader, device, sched_iter, learning_rate, num_epochs, noise_std, patience, min_delta,
    ...
    reco_net = Network_aacid(sched_iter, n_hidden=2, n_neurons=300).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for counter, dict_params in enumerate(train_loader, 0):
            ...
            target_norm = normalize_range(original_array=target, original_min=min_param_tensor,
                                     original_max=max_param_tensor, new_min=0, new_max=1)
            target = target_norm.to(device)

            input_aacid_ksw = target_norm[:, 1:]
            ...
            noised_sig = torch.hstack((input_aacid_ksw, input_mt_fs_ksw, input_water_t1t2, noised_sig))
        ...
        if epoch % 25 == 0:
            ...
            torch.save({
                ...
                'epoch': epoch,
                'input_order': ['ksw_aacid', 'mt_fs', 'mt_ksw', 't1', 't2'],
            }, nn_fn)
