...
def main(args):
    ...
    full_dataset = MTDataset(mt_dict_fn, args.norm_type, args.sched_iter)
...
def train_network(args, train_loader, val_loader, test_loader, net_name, nn_fn, min_max_params):
    ...
    reco_net = Network(args.sched_iter, add_iter=2, n_hidden=2, n_neurons=300, output_dim=2).to(args.device)
...
def train_step(args, reco_net, optimizer, cum_loss, dict_params, writer, epoch, counter, min_max_params, num_steps):
    ...
    (min_param_tensor, max_param_tensor,
     min_water_t1t2_tensor, max_water_t1t2_tensor) = min_max_params
    # dict params
    (cur_fs, cur_ksw,
     cur_t1w, cur_t2w,
     cur_mt_norm_sig) = dict_params
    ...
    noised_sig = torch.hstack((input_water_t1t2, mt_noised_sig.to(args.device))).to(args.device)
