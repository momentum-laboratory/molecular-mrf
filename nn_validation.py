...
def main(args):
    ...
    if args.scenario_type == '2pool':
        dict_fn = os.path.join(
    ...
    elif args.scenario_type == '3pool':
        dict_folder_fn = os.path.join(parent_dir, 'data', 'exp', 'mt_amide_glu_dicts',
                                      args.dict_name_category, 'glu', args.fp_prtcl_names[0])  # dict folder directory
        glu_memmap_fn = os.path.join(dict_folder_fn, 'dict.dat')
    ...
    yaml_file_path = os.path.join(os.path.dirname(dict_fn), 'scenario.yaml')
