...
from cest_mrf.metrics.dot_product_z import dot_prod_matching
...
class ConfigParams:
    ...
    water_t1 = np.arange(2300, 3500, 100) / 1000  # from ksw paper
    water_t1 = water_t1.tolist()  # vary t2
    # water_t1 = [3500 / 1000]

    # water_t2 = np.arange(1450, 1800, 50) / 1000  # from t2 maps (pbs)
    water_t2 = np.arange(1200, 1800, 50) / 1000  # from ksw paper
    ...
    cest_amine_k = np.arange(5000, 7600, 100).tolist()  # np.arange(2500, 8000, 100).tolist()  # vary glu exchange rate 7000-8000
    cest_amine_dw = 3

    cest_amine_sol_conc = np.arange(10, 11, 1)  # np.arange(0, 25, 1) glu concentration
...
def main():
    ...
    for phantom_i in range(len(glu_phantom_fns)):
        ...
        for fp_prtcl_name in fp_prtcl_names:
            ...
            seq_defs = {}
            # temp!!!
            _, _, bruker_dataset = bruker_dataset_creator(glu_phantom_fn, txt_file_name, 'EPI_4uT')
            seq_defs['n_pulses'] = 1                                             # number of pulses (1 in preclinical)
            seq_defs['tp'] = 3
            seq_defs['td'] = 0
            seq_defs['Trec'] = 5
            seq_defs['Trec_M0'] = 5
            seq_defs['M0_offset'] = 1000000 / (gyro_ratio_hz * b0)
            ...
            seq_defs['Tsat'] = (seq_defs['n_pulses'] *
                                ...
                                seq_defs['td'])                                  # (3 [s])
            seq_defs['B0'] = b0                                                  # B0 [T]
            seq_defs['FA'] = 90  # I don't actually know

            seqid = os.path.splitext(seq_fn)[1][1:]
            seq_defs['seq_id_string'] = seqid  # unique seq id

            # B1 variation
            seq_defs['B1pa'] = [4.0] * seq_defs['num_meas']
